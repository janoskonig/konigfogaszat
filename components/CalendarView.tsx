...
import { getAppointmentsByDateRange } from '@/lib/storage';
...
export function CalendarView({ onAppointmentClick }: CalendarViewProps) {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        const allAppointments = await getAppointmentsByDateRange(dateRange.start, dateRange.end);
        
        // Filter by status if selected
        let filtered = allAppointments;
        if (selectedStatus) {
          filtered = filtered.filter(apt => apt.appointmentStatus === selectedStatus);
        }
